...
class AdminView extends Component {
  ...
  static function print_label($name, $prefix='-input'){
    global $_SHOP;
     $rtn ='';
    if ($toolTipName = self::hasToolTip(strtolower($name))) {
      $hasTTClass = 'class="has-tooltip"';
      $toolTipText = con(strtolower($name)).'~'.con($toolTipName);
      $rtn = "&nbsp;<img src=\"{$_SHOP->images_url}help.gif\" alt=\"".con('help')."\" id='{$toolTipName}' class='has-tooltip' title='{$toolTipText}'>";
    }

    return '<label for="'.$name. $prefix.'">'.con(strtolower($name)).'</label>'.$rtn;
  }

  function print_field ($name, $data, $prefix='',$fieldtype='') {
      echo "<tr id='{$name}-tr'><td class='admin_name' width='".self::$labelwidth."'>$prefix" , self::print_label($name) , "</td>
          <td class='admin_value'>";
  ...
  function print_input ($name, &$data, &$err, $size = 30, $max = 100, $suffix = '', $arrPrefix='', $type = 'text') {
    ...
    echo "<tr id='{$name}-tr' ><td class='admin_name'  width='".self::$labelwidth."'>$suffix" . self::print_label($name) . "</td>
          <td class='admin_value'><input id='{$name}-input' type='{$type}' name='{$prefix}' value='" . htmlspecialchars(is($data[$name],''), ENT_QUOTES) . "' size='{$size}' maxlength='{$max}'>
          ".printMsg($name, $err)."
          </td></tr>\n";
  }
  ...
  function print_password ($name, &$data, &$err, $size = 30, $max = 100, $suffix = '', $arrPrefix='') {
    ...
    function print_checkbox ($name, &$data, &$err, $size = '', $max = '')
    {
      ...
      echo "<tr id='{$name}-tr'><td class='admin_name'>$suffix" . self::print_label($name,'-no') . "</td>
...
/*        if ($data[$name]) {
        ...
        echo "<tr><td class='admin_name'  width='".self::$labelwidth."'>" . self::print_label($name) . "</td>
    ...
    function print_area ($name, &$data, &$err, $rows = 6, $cols = 50, $suffix = '') {
      $suffix = self::_check($name, $suffix,$data);
      echo "<tr id='{$name}-tr'><td class='admin_name'>$suffix" . self::print_label($name, '-textarea') . "</td>
    ...
    function print_large_area ($name, &$data, &$err, $rows = 20, $cols = 80, $suffix = '', $options='') {
      ...
      $suffix = self::_check($name, $suffix,$data);
      echo "<tr id='{$name}-tr'><td colspan='2' class='admin_name'>$suffix" . self::print_label($name, '-textarea') . "&nbsp;&nbsp; ".printMsg($name, $err)."</td></tr>
    ...
    function print_set ($name, &$data, $table_name, $column_name, $key_name, $file_name) {
        ...
        echo "<tr id='{$name}-tr'><td class='admin_name'>" . self::print_label($name, '-a') . "</td>
    <td class='admin_value'>";
        if (!empty($set)) {
    ...
    function print_time ($name, &$data, &$err, $suffix = '') { // Lxsparks edited to allow for midday/midnight times
      global $_SHOP;
      $suffix = self::_check($name, $suffix,$data);
		...
		if (isset($data[$name])) {
          ...
          list($h, $m, $s) = explode(":", $src);
			...
			if (($_SHOP->input_time_type == 12) and ($h > 12)) {
				$h -=12;
				$data["$name-f"] = 'PM';
			} elseif (($_SHOP->input_time_type == 12) and ($h == 12)) {
				$data["$name-f"] = 'PM';
			}
      ...
      echo "<tr id='{$name}-tr'><td class='admin_name'>$suffix" . self::print_label($name, '-h') . "</td>
           <td class='admin_value'>
    ...
    function print_date ($name, &$data, $datetime= 'D', $suffix = '') {
      ...
      echo "<tr id='{$name}-tr'><td class='admin_name'>$suffix" . self::print_label($name, $pref) . "</td>
            <td class='admin_value'>".$echo;
      echo " ".printMsg($name)."
              </td></tr>\n";
    }

    function print_url ($name, &$data, $prefix = ''){
        echo "<tr id='{$name}-tr'><td class='admin_name' width='".self::$labelwidth."'>$prefix" . self::print_label($name,'-a') . "</td>
    <td class='admin_value'>
  ...
  function set_date($name,&$data, &$err) {
    ...
    function print_select ($name, &$data, &$err, $opt, $actions=''){
        ...
        echo "<tr id='{$name}-tr'><td class='admin_name'  width='".self::$labelwidth."'>" . self::print_label($name,'-select') . "</td>
              <td class='admin_value'>
        ...
        foreach($opt as $k => $v) {
          if (is_array($v)) {
            ...
            foreach($v as $v2 => $n2) {
              ...
              echo "<option value='". htmlspecialchars($v2)."' {$sel[$v2]}>" .  htmlspecialchars($cap) . "</option>\n";
            }
            echo "</optgroup>\n";
            continue;
          }
    ...
    function print_color ($name, &$data, &$err) {
        echo "<tr id='{$name}-tr'><td class='admin_name'  width='".self::$labelwidth."'>" . self::print_label($name,'-color') . "</td>
        <td class='admin_value'>";
    ...
    function print_file ($name, &$data, &$err, $type = 'img', $suffix = ''){
      ...
      $suffix = self::_check($name, $suffix,$data);
        if (!isset($data[$name]) || empty($data[$name])) {
            echo "\n<tr id='{$name}-tr'><td class='admin_name'  width='".self::$labelwidth."'>$suffix" . self::print_label($name,'-file') . "</td>
            <td class='admin_value'><input type='file' id='{$name}-file' name='{$name}'>".printMsg($name, $err)."</td></tr>\n";
        } else {
            $src = self::user_url($data[$name]);

            echo "<tr id='{$name}-tr'><td class='admin_name' rowspan=3 valign='top' width='".self::$labelwidth."'>$suffix" . self::print_label($name) . "</td>
            <td class='admin_value'>";
  ...
  global $_SHOP,  $_COUNTRY_LIST;
    ...
    echo "<tr id='{$name}-tr'><td class='admin_name'  width='".self::$labelwidth."'>" . self::print_label($name,'_select') . "</td>
    ...
    try{
      ...
      if (!$dlLink) {
        ...
        if(!isset($array['versions']['version']['0_attr'])){
          addwarning('Couldnt get version');
          return false;
        }
