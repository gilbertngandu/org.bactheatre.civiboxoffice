    ...
    echo "</tr>\n";
      ...
      echo  '&nbsp;'. showstr( $row['event_name'], 30 ) . "</nobr></td>\n";
        ...
        if (!$history) {
			...
			echo "<td width=80 class='admin_list_item' nowrap='nowrap'><nobr>";
...
//                                  onClick=\"window.location='view_event.php?action=edit&event_id={$row['event_id']}'\"
      ...
      echo "&nbsp;</nobr></td>
			...
			echo "<td class='admin_list_item' width=80 NOWRAP><nobr>";
      $this->showbuttons($img_pub, $row, $history);
			echo "
      </nobr></td></tr>\n\n";
			$alt = ( $alt + 1 ) % 2;
...
*/
  ...
  $collapsible =(!$data['event_id'] )?'false':'true';
  $this->addJquery('
	$(function() {
		$( "#accordion" ).accordion({
      autoHeight: false,
