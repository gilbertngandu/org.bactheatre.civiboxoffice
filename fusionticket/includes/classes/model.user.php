...
class User extends Model{
    ...
    if ($user->CheckValues($data, $status, $mandatory, $secure, $short)){
      ...
      if (ShopDB::begin('register user')) {
       ...
       // echo 'new status:',$status;
        ...
        if (in_array($status, array(2)) || (in_array($status, array(4)) && $_SHOP->posuser_activate)) {
  ...
  function delete() {
    ...
    if (ShopDB::begin('delete user')) {
       if (parent::delete()) {
         $query = 'delete from auth where user_id = '.esc($this->user_id);
    ...
    return self::_abourt('in_use_adminlink');
