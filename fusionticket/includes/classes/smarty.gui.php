 ...
 */
  ...
  function StartForm($params, $smarty) //($name, $width = 0, $colspan = 2)
  {
    ...
    If ($method <> 'none') {
      ...
      if (!$this->FormUsed) {
        $this->addJQuery(' $.metadata.setType("attr", "validate");');
        $this->FormUsed = true;
      }
  ...
  private function showlabel($name, $value = null, $params=array(), $colspan=1, $smarty=null) {
    ...
    if ($this->_ShowLabel and !$nolabel) {
      ...
      if ($toolTipName = $this->hasToolTip(strtolower($name))) {
        ...
        $rtn    = "&nbsp;<img src=\"{$_SHOP->images_url}help.gif\" alt=\"".con('help')."\" id='{$toolTipName}' style='vertical-align:middle;' class='has-tooltip' title='{$toolTipText}'>";
      }
