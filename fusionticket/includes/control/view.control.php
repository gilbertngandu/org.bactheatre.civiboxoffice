...
class ControlView extends AdminView{
  ...
  function checkBarcode(){
   ...
   $query="select * from Event
             where event_id = "._esc($_SESSION['event']);
    $event=ShopDB::query_one_row($query);
    echo "
      <center><br><h1>{$event['event_name']}</h1><br><br>
  ...
  function draw (){
    global $_SHOP;
 //  var_dump($_SHOP->event_ids, explode(',', $_SHOP->event_ids),in_array($_GET['event_id'],explode(',', $_SHOP->event_ids)));
    if($_GET['event_id'] && in_array($_GET['event_id'],explode(',', $_SHOP->event_ids))){
       $_SESSION['event']=$_GET['event_id'];
  	}

    if(!isset($_SESSION['event']) || $_GET['action']=="change_event"){
