...
//        $placemap .= "</div>";
			...
			$this->json['categories'][strval($cat['category_id'])] = array(
			  'html'=>$option, 'numbering'=>$numbering,
			  'placemap'=>$placemap,'price'=>$cat['category_price'],
			  'free_seats'=>$cat['category_free'],
			  'title'=>$cat['category_name']." -  ".$cat['category_price']);
		}
...
//		}
  ...
  //  			 "<input type='hidden' value='remove" name="action" />
    ...
    while ($pay=shopDB::fetch_assoc($res)){
      $fee = calculate_fee($pay, $subpricem, (!$mycart)?0:$mycart->count());
	...
	private function getPlaceMap(){
		...
		if(strtolower($cat['category_numbering']) != 'none'){
      $placemap = "<div style='overflow: auto; max-height: 450px; max-width:800px;' align='center' valign='center'>";
